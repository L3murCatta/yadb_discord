...
def roll(d, sign, mode, sort, count, fl):
    ...
    total *= sign
    res+= " = {}\n".format(("{}" if fl else "**{}**").format(total))
...
def rolldice(context):
    try:
        mode = modes[context.message.server.id + ' ' + context.message.channel.id]
    except Exception:
        modes[context.message.server.id + ' ' + context.message.channel.id] = 0
        mode = 0
    finally:        
        try:
            sort = sorts[context.message.server.id + ' ' + context.message.channel.id]
        except Exception:
            sorts[context.message.server.id + ' ' + context.message.channel.id] = 0
            sort = 0
        finally:
            try:
                count = counts[context.message.server.id + ' ' + context.message.channel.id]
            except Exception:
                counts[context.message.server.id + ' ' + context.message.channel.id] = 0
                count = 0
            finally:
                return (context.message.author.mention + ' ' + "" if mode == 0 else "{} mode:\n".format("Bad" if mode == -1 else "Good")) + parseandroll(context.message.content[context.message.content.find(' ')+1:], mode, sort, count)

@bot.command(pass_context = True,
                aliases = ['d'])
async def dice(context):
    await bot.say(rolldice(context))

@bot.command(pass_context = True)
async def sortf(context):
    global sorts
    if context.message.content.strip().find(' ') == -1:
        sorts[context.message.server.id + ' ' + context.message.channel.id] = 1
        await bot.say("Sorting enabled")
        return
    t = context.message.content[context.message.content.find(' ')+1:]
    if t == "d":
        sorts[context.message.server.id + ' ' + context.message.channel.id] = -1
        await bot.say("Descending sorting enabled")
        return
    if t == "off":
        sorts[context.message.server.id + ' ' + context.message.channel.id] = 0
        await bot.say("Sorting disabled")
        return
    sorts[context.message.server.id + ' ' + context.message.channel.id] = 1
    await bot.say("Sorting enabled")

@bot.command(pass_context = True)
async def countf(context):
    global counts
    if context.message.content.strip().find(' ') == -1:
        counts[context.message.server.id + ' ' + context.message.channel.id] = 1
        await bot.say("Counting enabled")
        return
    t = context.message.content[context.message.content.find(' ')+1:]
    if t == "raw":
        counts[context.message.server.id + ' ' + context.message.channel.id] = 2
        await bot.say("Couting raw enabled")
        return
    if t == "off":
        counts[context.message.server.id + ' ' + context.message.channel.id] = 0
        await bot.say("Counting disabled")
        return
    counts[context.message.server.id + ' ' + context.message.channel.id] = 1
    await bot.say("Counting enabled")
...
async def good(context):
    global modes
    modes[context.message.server.id + ' ' + context.message.channel.id] = 1
    await bot.say("Commencing good mode")

@bot.command(pass_context = True)
async def normal(context):
    global modes
    modes[context.message.server.id + ' ' + context.message.channel.id] = 0
    await bot.say("Commencing normal mode")

@bot.command(pass_context = True)
async def bad(context):
    global modes
    modes[context.message.server.id + ' ' + context.message.channel.id] = -1
    await bot.say("Commencing bad mode")

@bot.command(pass_context = True)
async def f(context):
    t = context.message.content
    n, t = parsex(t)
    ...
    if t.strip().find(" ") != -1:
        try:
            ...
            await bot.say("Bad syntax")
            return
    if x > 0:
        res += "+"
    if x != 0:
        res += str(x)
    context.message.content = res
    await bot.say(rolldice(context))
...
@bot.command(pass_context = True)
async def g(context):
    n, _ = parsex(context.message.content)
    context.message.content = "/dice x{} 3d6".format(n)
    await bot.say(rolldice(context))

@bot.command(pass_context = True)
async def p(context):
    n, _ = parsex(context.message.content)
    context.message.content = "/dice x{} d100".format(n)
    await bot.say(rolldice(context))
...
async def n(context):
    ...
    try:
        ...
        await bot.say("Bad syntax")
        return
    context.message.content = "/dice x{} {}d10t>=8!>={}".format(n, x, y)
    await bot.say(rolldice(context))
...
async def register(context):
    global teams
    cont = context.message.content.strip().split()[1:]
    if len(cont) != 2:
        await bot.say(context.message.author.mention + " I need name and race for that")
        return
    if cont[0] == "me":
        cont[0] = context.message.author.name
    try:
        teams[context.message.server.id + ' ' + context.message.channel.id].append(cont+[20])
    except Exception:
        teams[context.message.server.id + ' ' + context.message.channel.id] = [cont+[20]]
    await bot.say(cont[0] + " registered")

##@bot.command(pass_context = True)
##async def state(context):
##    try:
##        t = teams[context.message.server.id + ' ' + context.message.channel.id]
##        l = len(t)
##        if l == 0:
##            await bot.say("No players")
##        else:
##            for s in range(l):
##                await bot.say("**{}** - **{}**".format(t[s][0], t[s][1]))
##                await bot.say(":heart: **HP**: {}/20".format(t[s][2]))
##                if s < l-1:
##                    await bot.say("--------")
##    except Exception:
##        await bot.say("No players")
...
async def state(context):
    ...
    try:
        ...
        if l == 0:
            ...
            for s in range(l):
                msg += "**{}** - **{}**".format(t[s][0], t[s][1])
                msg += '\n'
                msg += ":heart: **HP**: {}/20".format(t[s][2])
                msg += '\n'
                if s < l-1:
                    msg += "--------"
                    msg += '\n'
...
async def deal(context):
    cont = context.message.content.strip().split()[1:]
    if len(cont) != 2:
        await bot.say(context.message.author.mention + " I need name and damage for that")
        return
    for s in teams[context.message.server.id + ' ' + context.message.channel.id]:
        if s[0] == cont[0]:
            s[2] -= int(cont[1])
            await bot.say("Damage dealt")
            break
    else:
        await bot.say("There is no such character")

@bot.command(pass_context = True)
async def heal(context):
    cont = context.message.content.strip().split()[1:]
    if len(cont) != 2:
        await bot.say(context.message.author.mention + " I need name and damage for that")
        return
...
async def clean(context):
    teams[context.message.server.id + ' ' + context.message.channel.id] = []
    await bot.say("Session finished")
